...
DIR_BASE = os.path.dirname (os.path.abspath(__file__)) # Ruta en la que se encuentra este script
...
def procesar_archivos (archivos):
    """
    Procesa los archivos y distribuye la carga (round-robin) entre los servicios, actualizando los logs.
    ...
    for archivo in archivos:
        nombre_archivo = os.path.basename(archivo) # Solo el nombre del archivo, no una ruta
        ...
        try:
            ...
            i_servicio = (i - 1) % NUM_SERVICIOS + 1 # Numero de servicio que maneja la solicitud
            carpeta_destino = os.path.join (DIR_BASE, f"service_{i_servicio}")
            os.makedirs(carpeta_destino, exist_ok=True)

            timestamp_archivos = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
            ...
            os.rename (ruta_archivo, os.path.join(carpeta_destino, nuevo_nombre_archivo))

            # Actualización de logs en cada carpeta de service_<id>/
            timestamp_logs = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            actualizar_log(i_servicio, nuevo_nombre_archivo, timestamp_logs)

        except Exception as e:
            print (f"No se pudo procesar el archivo '{nombre_archivo}': {e}. Moviendo a \"errors\"...")
